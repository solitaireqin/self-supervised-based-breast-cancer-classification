...
class Densenet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.linear(out).squeeze()
            # 如果 linear1, linear2, linear3 是动态添加的，使用 getattr 调用
        if hasattr(self, 'linear1'):
            out = F.relu(getattr(self, 'linear1')(out))  # 降至 128
        if hasattr(self, 'linear2'):
            out = F.relu(getattr(self, 'linear2')(out))  # 降至 16
        if hasattr(self, 'linear3'):
            out = getattr(self, 'linear3')(out)  # 最终输出 2
