...
from pretrain.utils import HardTripletloss,get_indices, InfoNCEloss
...
def train(model):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        average_loss = trainloss / iter
        loss_log.append({'Epoch': epoch + 1, 'Loss': average_loss})
        # 打印每个 epoch 的平均损失
        print(f"Epoch [{epoch + 1}/{epochs}] Average Loss: {trainloss / iter:.4f}")
        if epoch % 1 == 0:
            trainloss = 0
    # 将 loss_log 保存到 CSV 文件
    loss_df = pd.DataFrame(loss_log)
    loss_csv_path = os.path.join(args.save_models_dir, 'training_loss.csv')
