...
class LoadDatasets(Dataset):
    ...
    def sample(self, i, f1, f2, b1, b2):
        ...
        M = transforms.functional.to_pil_image(Mean)
    ...
    def __getitem__(self, index):
        ...
        patient, video, shot = imagePath.split('_')
        ...
        if len(self.d[int(patient)]) != 1:
            ...
            negative_labels = random.sample(list(self.d.keys()), self.choose_neg)
            #####可能问题在于负样本的获取
            for label in negative_labels:
                mid = 1
                # 调整帧编号的采样逻辑
                last = np.random.choice(range(1, self.d[label][mid]+1))
